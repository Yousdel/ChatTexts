...
const QString ConcreteGUIChatTexts::getHTMLfor(const QString &s)
{
    QString htmlFor;
...
const QString ConcreteGUIChatTexts::workTextTypeChat(const QString &text)
{
    if (text.contains(": ")==0)
        return QString();
...
ConcreteGUIChatTexts::ConcreteGUIChatTexts()
{

}

const QString &ConcreteGUIChatTexts::getHTML()
{
    const QStringList sN = text().split("\n");
    html = "<html><head/><body>";
    bool newMsg=0, firstNewMsg=0;
    for (const QString &ss : sN)
        if (ss.isEmpty()) continue; else
        {
